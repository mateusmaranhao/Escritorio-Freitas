...
export function Header() {
  ...
  return (
    <header 
      className={clsx(
        'w-full h-20 px-6 lg:px-12 border-b border-[#333333]/10 flex items-center justify-between sticky top-0 bg-brand-surface z-50 transition-all duration-300',
        isScrolled ? 'shadow-sm' : ''
      )}
    >
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 bg-brand-primary flex items-center justify-center">
          <span className="text-brand-surface font-bold text-xs">F</span>
        </div>
          ...
          <AnimatePresence>
            {servicesOpen && (
              <motion.div 
                ...
                className="absolute top-full left-0 mt-0 w-64 bg-brand-surface border border-brand-primary/10 shadow-sm"
              >
                <ul className="py-2">
                  {serviceLinks.map((link) => (
                    <li key={link.path}>
                      <Link 
                        to={link.path} 
                        className="block px-6 py-3 font-gilroy text-[11px] uppercase tracking-widest text-brand-secondary hover:text-brand-primary hover:bg-black/5 transition-colors"
                      >
                        {link.name}
                      </Link>
                    </li>
      ...
      <div className="flex items-center gap-4">
        <a 
          href="/contato" 
          className="hidden lg:inline-block border border-brand-primary px-6 py-2 text-[10px] uppercase tracking-[0.2em] font-semibold text-brand-primary hover:bg-brand-primary hover:text-brand-surface transition-all duration-300"
        >
          Agendar Consultoria
        </a>
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col px-6 py-4 gap-4">
              <Link to="/sobre" className="font-gilroy text-[14px] uppercase tracking-widest py-2 border-b border-brand-primary/5">
                SOBRE
              </Link>
              <div className="py-2 border-b border-brand-primary/5">
                <span className="font-gilroy text-[14px] uppercase tracking-widest block mb-4 text-brand-primary">SERVIÇOS</span>
                <ul className="flex flex-col gap-3 pl-4">
                  {serviceLinks.map((link) => (
                    <li key={link.path}>
                      <Link to={link.path} className="font-gilroy text-[14px] text-brand-secondary block">
                        {link.name}
                      </Link>
                    </li>
                  ))}
              ...
              <Link to="/contato" className="font-gilroy text-[14px] uppercase tracking-widest py-2">
                CONTATO
              </Link>
            </nav>
          </motion.div>
